...
- **Accessibility smoke test**: New 12-case `AccessibilitySmokeTest` runs in plain PHPUnit (no browser needed on CI) — icon-only controls need `aria-label` (controls with visible text exempt), no global `outline: none`, buttons need an accessible name or visible text, form inputs need `<label for>` or their own `aria-label`, embed declares `lang="en"` and index/editor inherit it via the LibreNMS layout, index modals use `role="dialog"`, and the embed map canvas got a real `aria-label` as its programmatic text alternative.
- **Nested maps / drill-down hierarchy**: `parent_map_id` on `wmng_maps` (hasColumn-guarded migration plus all `setup.php` seeding paths) with `Map::parentMap()` / `childMaps()` / cycle-bounded `breadcrumb()`. Nodes can drill down via `meta.sub_map_id`; the `toJsonModel()` payload carries `parent_map_id`, `breadcrumb`, and per-node `sub_map_id`. The embed view navigates to a sub-map when a drill-down node is clicked (taking priority over device click-through) and renders a root→current breadcrumb bar. Six test cases in `NestedMapTest`.
