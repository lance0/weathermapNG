...
- **Dead FormRequest and cache classes**: removed `CreateLinkRequest`, `CreateNodeRequest` (never type-hinted by any controller — `MapLinkController`/`MapNodeController` use inline `$request->validate()`), and `MapCacheService` (zero callers; eager-load fixes it once held were superseded by the v1.7.0 N+1 elimination). The stale "10 Gbps" cap message fixed in PR #25 lived in `CreateLinkRequest`; removing the class retires it for good. The `CreateNodeRequest` `sanitize` custom validation rule was never registered, so the class would have failed at runtime had it been injected. `SanitizationTest` keeps its own `sanitizeNodeData` helper (the class was only referenced by a comment).
